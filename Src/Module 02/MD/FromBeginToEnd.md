...
In this module we'll transfer and restructure all the layout and code we built in Module 01 for the Standalone Web Page.

We will see the changes our code and layout go through, and the new tools that Microsoft provided for our Control Addiin development.

Firstly, it's important to recognize how a Control Add-in gets **inserted** into a Business Central Page. Business Central creates an `iframe` in the `Document Object Model` and names it `id="controlAddIn"`.
...
In other words, as we start our **Control Add-in** design, we must keep in mind that everything we build must work within an `iframe`. (This means we cannot access the rest of the DOM and do any mischief.) Microsoft really did place our control add-ins behind a cage.

In terms of stages of running code, the following is a good road map:
* Wait for everything to load (that means placing our intantiation code in `startup.js`)
* In `startup.js`, first we send our `HTML` scaffolding and insert it inside the `<div>` with the `controlAddIn` ID.
* We don't have to do anything for this to happen since we'll define the `DataTable.js` and `data.js` and `custStyle.css` files as links in the `iDataTable.controladdin.al` file, but our code will then call the function that attaches the `DataTable` widget to our `HTML <table>`.

We'll see this as it all falls together.

Let's start.

## Building a Business Central Extension

This time we're writing a Business Central Extension. That's the primary approach for Control Add-ins. We build the extension first and populate the Control Add-in logic later. 
...
### Other Files
JavaScript and a Cascading Style Sheet round out the *ControlAddIn* folder:
...
are the Javascript files in the folder. **data.js** has been copied, intact, from Module 01. It is the only JavaScript file so untouched.

* `custStyle`

is the sole stylesheet and it, too, come unchanged from Module 01.
...
````
A page that holds a `usercontrol` is a common Page type. What's different is the introduction of a new page part.
...
Firstly, there's a file we haven't filled: `startup.js`. Business Central calls this file (that we told it was the `StartupFile` in the ControlAddIn object file). The file is a JavaScript file and uses the new `InvokeExtensibilityMethod` provided by Microsoft for this very purpose.
...
The second line will call a `Trigger` on the `usercontrol` on the `iTable` `usercontrol`. 
